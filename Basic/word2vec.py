...
#prepocessing the para
import re
text  = re.sub('[^a-z,A-Z]', ' ', para)
text = re.sub(r'\[[0-9]*\]',' ',para)
text = re.sub(r'\s+',' ',text)
text = re.sub(r'\d', ' ', text)
text = re.sub(r'\s+',' ', text)
text = text.lower()
#conversion of text to tokenizer-> word tokenizer
sentence = nltk.sent_tokenize(text)
sentence = [nltk.word_tokenize(sentences) for sentences in sentence]
for i in range(len(sentence)):
    sentence[i] = [words for words in sentence[i] if words not in set(stopwords.words('English'))]
#training the model
